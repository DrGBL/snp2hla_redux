...
from MakeGeneticMap.src.RUN_Bash import RUN_Bash
from MakeGeneticMap.Panel_subset import Panel_Subset

from MakeGeneticMap.src.checkInput import getSampleNumbers
...
def MakeGeneticMap(_input, _reference, _out, _mach_mcmc_rounds,
    ...
    VCF2BEAGLE = 'java -jar {}'.format(_p_vcf2beagle)
    RANDOMIZE_FAM = 'Rscript {}/STEP0_randomize_the_sample_about_fam_03_06_2017-COOK-V1.R'.format(_p_src)
    GBL_BGL2GC_TRICK_BGL = 'Rscript {}/GBL_BGL2GC_TRICK_BGL.R'.format(_p_src)
    ...
    if f_SmallSampleMode:
        ...
        if SUBSET_BGL:
            ...
            RUN_Bash('awk \'{print $2" "$4" "$6" "$5}\' %s > %s' % (OUTPUT_INPUT+'.subset.bim', OUTPUT_INPUT+'.subset.markers'))
            ...
            RUN_Bash(GBL_BGL2GC_TRICK_BGL + ' {} {} {} {} {} {} {} {}'.format(
                OUTPUT_INPUT + '.subset.bgl.phased', OUTPUT_INPUT+'.subset.markers',
                OUTPUT_REF + '.subset.bgl.phased', OUTPUT_REF+'.subset.markers',
                OUTPUT_INPUT + '.subset.GCchange.bgl.phased', OUTPUT_INPUT + '.subset.GCchange.markers',
                OUTPUT_REF + '.subset.GCchange.bgl.phased', OUTPUT_REF + '.subset.GCchange.markers'))
    ...
    if RUNMACH:
        ...
        RUN_Bash(_p_mach+' -d {} -p {} -h {} -s {} --rounds {} --greedy --prefix {}'.format(
    ...
    if Cleanup:
        ...
        RUN_Bash('rm {}'.format(OUTPUT_INPUT+'.bed'))
        RUN_Bash('rm {}'.format(OUTPUT_INPUT+'.bim'))
        RUN_Bash('rm {}'.format(OUTPUT_INPUT+'.fam'))
