...
from MakeGeneticMap.src.checkInput import getSampleNumbers, exclude_Ambiguous_SNP, FixInput
...
#not using _answer or _f_BEAGLE5?
def CookHLA(_input, _hg_input, _out, _reference, _mach_mcmc_rounds, _hg_reference='38', _AdaptiveGeneticMap=None, _Average_Erate=None,
            _java_memory='2g', _MultP=1, _answer=None, __save_intermediates=True, __use_Multiple_Markers=True,
            _p_src="./src", _p_dependency="./dependency", _given_prephased=None, f_prephasing=False, _HapMap_Map=None,
            __overlap__=(0.5, 1, 1.5), _window=5, _ne=10000, _nthreads=1, _burnin=3, _iter=12, f_BEAGLE5=False,
    ...
    if not(which("plink") is None) and exists(which("plink")):
...
#        _p_beagle5 = None
    ...
    _p_beagle5 = join(p_dependency, 'beagle.jar')

    if not(which("tcsh") is None) and exists(which("tcsh")):
        _p_tcsh = which('tcsh')
    ...
    if not(which("perl") is None) and exists(which("perl")):
        _p_perl = which('perl')
    else:
...
#        BEAGLE5 = None
    ...
    if _Average_Erate and _AdaptiveGeneticMap:
        ...
        [_AdaptiveGeneticMap, _Average_Erate] = MakeGeneticMap(_input, _reference, join(OUTPUT_dir, 'AGM.{}+{}'.format(basename(_input), basename(_reference))), _mach_mcmc_rounds)
...
#    if f_BEAGLE5:
    ...
    if f_SmallSampleMode:
        ...
        if EXTRACT_MHC:
            ...
            command = ' '.join([PLINK, '--bfile', _input, '--chr 6', '--from-mb 28 --to-mb 34', '--make-bed', '--out', MHC]) # '--maf 0.025' is not used due to 'Small Sample mode'.
            # print(command)
            os.system(command)
        ...
        if EXTRACT_MHC:
            ...
            #command = ' '.join([PLINK, '--bfile', _input, '--chr 6', '--from-mb 29 --to-mb 34', '--maf 0.025', '--make-bed', '--out', MHC])
            command = ' '.join([PLINK, '--bfile', _input, '--chr 6', '--from-mb 29 --to-mb 34', '--maf 0.005', '--make-bed', '--out', MHC])
            # print(command)
            os.system(command)
...
#                                          _AdaptiveGeneticMap, _Average_Erate, 
#                                          LINKAGE2BEAGLE, BEAGLE2LINKAGE, BEAGLE2VCF, VCF2BEAGLE, PLINK, BEAGLE5, _answer=_answer,
...
if __name__ == "__main__":
    ...
    # Make Genetic Map
    parser.add_argument("--mach_mcmc_rounds", "-r", help="\nNumber of MCMC rounds for mach1. Default=20.\n\n", default=20) 
    ...
    parser.add_argument("--overlap", "-ol",
                        help="\n3 Overlap values(cM) for Beagle 5.4 implementation.\n\n", nargs=3, default=(0.5, 1, 1.5), type=float)
    parser.add_argument("--window", "-w",
                        help="\nWindow value(cM) for Beagle 5.4 implementation.\n\n", default=5, type=float)
    parser.add_argument("--effective-population-size", "-ne",
                        help="\nEffective population size value for Beagle 5.4 implementation.\n\n", default=10000, type=int)
    ...
    CookHLA(_input=args.input, _hg_input=args.human_genome, _out=args.out, _reference=args.reference, _mach_mcmc_rounds=args.mach_mcmc_rounds, 
            _hg_reference="38", _AdaptiveGeneticMap=args.genetic_map, _Average_Erate=args.average_erate,
            _java_memory=args.java_memory, _MultP=args.multiprocess, _answer=args.answer, __overlap__=args.overlap,
            _window=args.window, _ne=args.effective_population_size, _nthreads=args.nthreads, _burnin=args.burnin, _iter=args.iter,
