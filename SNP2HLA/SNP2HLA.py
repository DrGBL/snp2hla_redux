...
def SNP2HLA(_target, _reference, _out, _maf="0.01", _mem="2g", _tolerated_diff=.15, _p_dependency="./dependency",
            _b_nthreads=1,
            _burnin=3, _iter=12, _map="null", _window=2.8, _overlap=1.25):
    ...
    if EXTRACT_MHC:
        ...
        #MAF imputation threshold
        command = ' '.join([PLINK, "--bfile", _target, "--chr 6", "--from-mb 28 --to-mb 34", "--maf", _maf, "--make-bed", "--out", __MHC__])
        # print(command)
        os.system(command)
    ...
    if IMPUTE:
        ...
        if _map=="null":
          command = ' '.join([BEAGLE,
                            ...
                            "burnin={}".format(_burnin), 
                            "iterations={}".format(_iter), "window={}".format(_window), "overlap={}".format(_overlap)])
        else:
          command = ' '.join([BEAGLE,
                            "gt={}".format(__MHC__+".QC.bgl.vcf"),
                            'ref={}'.format(_reference + ".bgl.phased.vcf.gz"),
                            ...
                            'out={}'.format(OUTPUT +".bgl.phased"),
                            "burnin={}".format(_burnin), 
                            "iterations={}".format(_iter),
                            "map={}".format(_map), "window={}".format(_window), "overlap={}".format(_overlap)])
