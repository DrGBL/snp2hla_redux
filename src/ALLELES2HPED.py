...
p_4digit = re.compile(r'[0-9]+:[0-9]+')
p_4digit_pair = re.compile(r'^[0-9]+:[0-9]+,[0-9]+:[0-9]+$')
p_2digit_pair = re.compile(r'^[0-9]+,[0-9]+$')
...
#     print("df_alleles :\n{}\n".format(df_alleles.head()))
    ...
    f_4digit = df_alleles.astype(str).applymap(lambda x : (bool(p_4digit_pair.match(x)) and (not bool(re.match(r'^,$', x))))).apply(lambda x : x.any(), axis=0)
    
    #tmp = df_alleles[[4,5]]
    #tmp.applymap(lambda x : (bool(p_4digit_pair.match(x)) and (not bool(re.match(r'^,$', x))))).apply(lambda x : x.any(), axis=0)
#     print(f_4digit)
