...
def Make_EXON234_AGM(_GM, _REF_exon234_markers, _out):
    ...
    #df_merged3 = GEN_stitch_GD(df_merged2)
    #df_merged3.columns = ['Chr', 'ID', 'GD','BP']
    #if df_merged3.loc[df_merged3.shape[0]-1, 'GD'] == 0:
    #  df_merged3.loc[df_merged3.shape[0]-1, 'GD'] = df_merged3.loc[df_merged3.shape[0]-2, 'GD']
...
def GEN_Trim_Columns(_df_merged):
    ...
    for row in _df_merged.itertuples():
        ...
        if not p_HLA.match(_ID_):
            yield ['6', _ID_, row[3], _BP_] # ex) '6	rs9277935	9.094223819100000	33268403'
        else:
...
def GEN_stitch_GD(_df):
    ...
    while i < n_rows:
        ...
        if _GD_[i-1] != 0 and _GD_[i] == 0:
            ...
            if end == n_rows - 1 and float(l_rows[end_cap_SNP][3]) == 0:
              mid = float(l_rows[start_cap_SNP][3])
            else:
